...
import bhawanaImage from '../assets/bhawana-kc.jpg';
...
export default function Team() {
  // Team Data - ALL 7 MEMBERS with corrected names
  const team = [
    { name: "Mukesh Yadav", title: "Founder, CEO & Creative Director", image: mukeshImage },
    { name: "Prakash Pant", title: "Manager", image: prakashImage },
    { name: "Bhawana KC", title: "Social Media Manager", image: bhawanaImage },
  ...
  return (
    <div className="w-full bg-slate-50 dark:bg-slate-950">
      ...
      {/* TEAM SECTION - INCREASED BREADTH */}
      <div className="w-full py-20 md:py-24">
        <div className="max-w-[1800px] mx-auto px-4 sm:px-8 lg:px-12">
          ...
          <div className="text-center mb-12 md:mb-16">
            ...
            <h2 className="text-3xl md:text-5xl font-bold text-slate-900 dark:text-white mb-3">Meet Our Team</h2>
            <p className="text-base md:text-lg text-slate-500 dark:text-slate-400 max-w-3xl mx-auto">
              The creative minds and technical experts behind MotionAge's success.
            </p>
          </div>
          
          {/* Team Grid - Increased width and spacing */}
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7 gap-6 md:gap-8 lg:gap-10">
            {team.map((member, index) => (
              ...
              >
                ...
                <div className="w-full max-w-[200px] mx-auto aspect-square overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 dark:from-slate-800 dark:to-slate-900 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 transition-all duration-300 group-hover:shadow-xl group-hover:scale-105 group-hover:border-blue-500/50">
                  <img 
                    src={member.image} 
                    alt={member.name} 
                ...
                {/* Name - Larger text */}
                <h4 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white mt-4 mb-1 text-center">
                  {member.name}
                </h4>
                
                {/* Title - Wrapped for better readability */}
                <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 text-center leading-relaxed px-2">
                  {member.title}
                </p>
              </motion.div>
            ))}
          ...
          <div className="text-center mt-16 pt-8 border-t border-slate-200 dark:border-slate-800">
            <p className="text-slate-600 dark:text-slate-400 mb-4">Want to be part of our creative collective?</p>
            <Link 
              to="/careers" 
              className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-blue-600 to-purple-600 text-white rounded-full font-bold text-sm hover:shadow-2xl hover:scale-105 transition-all duration-300"
            >
              Join Our Team
...
function ServiceStage({ stage }) {
  ...
  return (
    <div ref={ref} className="h-[100svh] relative flex items-center overflow-hidden border-b border-slate-200 dark:border-slate-900">
      ...
      <div className={`relative z-10 w-full flex ${isTextOnRight ? 'justify-end' : 'justify-start'} px-6 sm:px-8 lg:px-16 xl:px-24 max-w-[1800px] mx-auto`}>
